...
const content = {
  en: {
    ...
    leadership: [[photos.leader, "Ato Melaku Adane", "Founder & General Manager"], [photos.office, "Construction management", "Office & project coordination"], [photos.siteTeam, "Field team", "Site supervision & execution"]],
    ...
    name: "Full name", email: "Email address", phone: "Phone number", type: "Project type", location: "Project location", message: "Project scope and requirements", submit: "Prepare inquiry",
  ...
  am: {
    ...
    leadership: [[photos.leader, "አቶ መልኩ አዳነ", "መስራችና ጠቅላላ ሥራ አስኪያጅ"], [photos.office, "የግንባታ አስተዳደር", "ቢሮና የፕሮጀክት ማስተባበር"], [photos.siteTeam, "የሳይት ቡድን", "የሳይት ክትትልና አፈጻጸም"]],
    ...
    name: "ሙሉ ስም", email: "ኢሜይል", phone: "ስልክ", type: "የፕሮጀክት አይነት", location: "የፕሮጀክት ቦታ", message: "የስራ ወሰንና መስፈርቶች", submit: "ጥያቄ ያዘጋጁ", success: "ጥያቄዎ ተዘጋጅቷል። ለቀጥታ መላክ የድርጅቱን የተረጋገጠ ኢሜይል ማገናኘት ያስፈልጋል።",
...
const gallery = [
  ...
  [photos.graderTruck, "Grader and truck"], [photos.buildingSite, "Building frame"], [photos.event, "Company event"], [photos.office, "Office"], [photos.completedPath, "Completed public space"], [photos.leader, "Leadership"],
];
...
export default function Home() {
  ...
  return <main id="top" className="min-h-screen bg-[#f7f8fa] text-slate-950">
    ...
    <section id="leadership" className="mx-auto max-w-7xl px-5 py-20 lg:py-28"><p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500">{t.leadershipEyebrow}</p><h2 className="mt-3 text-3xl font-black tracking-[-0.035em] sm:text-4xl">{t.leadershipTitle}</h2><p className="mt-4 max-w-2xl text-base leading-8 text-slate-600">{t.leadershipText}</p><div className="mt-10 grid gap-6 lg:grid-cols-3">{t.leadership.map(([src,name,title]) => <article key={String(name)} className="overflow-hidden rounded-3xl border border-slate-200 bg-white"><div className="relative aspect-[4/4.1] bg-slate-200"><Image src={src} alt={String(name)} fill sizes="(max-width: 1024px) 100vw, 33vw" className="object-cover"/></div><div className="p-6"><h3 className="text-lg font-extrabold">{String(name)}</h3><p className="mt-1 text-sm font-semibold text-slate-500">{String(title)}</p></div></article>)}</div></section>
